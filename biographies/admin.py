...
class BiographyAdmin(SummernoteModelAdmin):
    ...
    def biography_link(self, obj):
        return mark_safe(f'<a href="/biographies/{obj.slug}" target="_blank">/biographies/{obj.slug}</a>')
    ...
    fieldsets = (
         ...
         ('ADDITIONAL INFORMATION', { 'fields'   : ['external_links', 'references', 'revisions'], 'classes': ['collapse'] }),
    )
