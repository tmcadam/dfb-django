...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not options['dry_run']:
            bio.save()
            self.stdout.write(self.style.SUCCESS(f"Slug field updated: {old_slug} -> {bio.slug}"))
        else:
            self.stdout.write(self.style.WARNING(f"Dry run: Slug field would be updated: {old_slug} -> {bio.slug}"))

        old_url = f"\"/biographies/{old_slug}\"".replace(" ", "%20")
        new_url = f"\"/biographies/{bio.slug}\""

        q = Q(body__icontains=old_url) | Q(references__icontains=old_url) | Q(external_links__icontains=old_url)
        ...
        self.stdout.write(self.style.SUCCESS(f"Links found: bios({bio_update_count}), images({image_update_count})"))
        ...
        if not options['dry_run']:
            Biography.objects.all().update(
                ...
                external_links=Replace('external_links', Value(old_url), Value(new_url))
            )
            Image.objects.all().update(
                caption=Replace('caption', Value(old_url), Value(new_url))
            )
            self.stdout.write(self.style.SUCCESS(f"Links updated: {old_url} -> {new_url}"))
        else:
            self.stdout.write(self.style.WARNING(f"Dry run: Links would be updated: {old_url} -> {new_url}"))
